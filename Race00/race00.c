...
void draw_vertical_borders(int x_max);
void draw_strings(int x_max, int y_max, int one_y, int one_x);

void draw_strings(int x_max, int y, int one_y, int one_x) {
...
void draw_vertical_borders(int x_m) {
	for (int i = 0; i < x_m; i++) {
		if (x_m == 1)
			mx_printstr("<=>");
		else if ( i == 0) {
...
void race00(int map_length, int map_width, int one_y, int one_x) {
	...
	&& map_width > 0 && map_length > 0) {
		draw_vertical_borders(map_width);
		mx_printchar('\n');
